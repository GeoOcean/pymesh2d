...
def tridiv(vert=None, conn=None, tria=None, tnum=None, tdiv=None):
    """
    tridiv "quadtree" refinement for 2-simplex triangulations.
    Python translation of Darren Engwirda's tridiv.m
    with 0-based indexing and safe concatenation.
    """
    ...
    # mark edges belonging to triangles to be refined
    ...
    tr41 = np.column_stack([tria[div3_idx, 0],
                            ivec[tria[div3_idx, 3]],
                            ivec[tria[div3_idx, 5]]])
    tr42 = np.column_stack([ivec[tria[div3_idx, 3]],
                            tria[div3_idx, 1],
                            ivec[tria[div3_idx, 4]]])
    tr43 = np.column_stack([ivec[tria[div3_idx, 5]],
                            ivec[tria[div3_idx, 4]],
                            tria[div3_idx, 2]])
    tr44 = np.column_stack([ivec[tria[div3_idx, 5]],
                            ivec[tria[div3_idx, 3]],
                            ivec[tria[div3_idx, 4]]])
    ...
    tr21 = np.column_stack([ivec[tria[tvec, 3]],
                            tria[tvec, 2],
                            tria[tvec, 0]]) if np.any(tvec) else np.empty((0, 3), int)
    tr22 = np.column_stack([ivec[tria[tvec, 3]],
                            tria[tvec, 1],
                            tria[tvec, 2]]) if np.any(tvec) else np.empty((0, 3), int)
    tn21 = tnum[tvec]
    tn22 = tnum[tvec]

    #---------------------------------------------- 1-to-2 refinement edge 2
    tvec = ediv[tria[:, 4]] & div1
    tr23 = np.column_stack([ivec[tria[tvec, 4]],
                            tria[tvec, 0],
                            tria[tvec, 1]]) if np.any(tvec) else np.empty((0, 3), int)
    tr24 = np.column_stack([ivec[tria[tvec, 4]],
                            tria[tvec, 2],
                            tria[tvec, 0]]) if np.any(tvec) else np.empty((0, 3), int)
    tn23 = tnum[tvec]
    tn24 = tnum[tvec]
    ...
    tr25 = np.column_stack([ivec[tria[tvec, 5]],
                            tria[tvec, 1],
                            tria[tvec, 2]]) if np.any(tvec) else np.empty((0, 3), int)
    tr26 = np.column_stack([ivec[tria[tvec, 5]],
                            tria[tvec, 0],
                            tria[tvec, 1]]) if np.any(tvec) else np.empty((0, 3), int)
    tn25 = tnum[tvec]
    tn26 = tnum[tvec]
